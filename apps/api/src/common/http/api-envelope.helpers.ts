...
} from "./api-envelope.types.js";
import { RAW_RESPONSE_METADATA } from "./raw-response.decorator.js";
...
export function isRawResponsePath(request: HttpRequestLike): boolean {
  return getRequestPath(request) === "/health";
}
...
export function ensureRequestId(request: HttpRequestLike, response: HttpResponseLike): string {
...
export function createMeta(request: HttpRequestLike, requestId: string): ApiEnvelopeMeta {
  return {
    requestId,
...
export function toPaginationMeta(payload: PaginatedPayload): ApiPaginationMeta {
  const totalPages = payload.pageSize > 0 ? Math.ceil(payload.total / payload.pageSize) : 0;
...
export function normalizeErrorDetails(errors: unknown, fallbackCode: string): ApiErrorDetail[] | undefined {
  ...
  return errors.map((error) => {
    if (isRecord(error)) {
      ...
      const code =
        typeof error.code === "string" ? error.code : field ? "VALIDATION_ERROR" : fallbackCode;
